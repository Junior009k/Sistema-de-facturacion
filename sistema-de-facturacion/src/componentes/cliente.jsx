        ...
        {   
          ...
          Swal.fire({
            ...
            if (result.isConfirmed) 
            {
              ...
              const actualizacionRef = doc(db, "cliente", e.target.value);
...
//DELETE: elimina los datos de la tabla inventario
  const eliminar=(e,nombre)=>
  {
    Swal.fire({
      ...
      if (result.isConfirmed) 
      {
        deleteDoc(doc(db,'cliente', e.target.value))
        Swal.fire(
  ...
  useEffect(()=>
  {
    ...
    return (
      <div className="cliente">
          ...
          <Button value={a} className="button" variant="primary" onClick={(e)=>{handleShow();}}>Agregar cliente</Button>
